...
def ensureConfigExists():
    ...
    try:
        ...
        print('[mod_aimpro_mdv133] config.json created with defaults')
    except Exception as e:
        print('[mod_aimpro_mdv133] Failed to create config.json:', e)
...
def loadConfig():
    ...
    try:
        ...
        print('[mod_aimpro_mdv133] Failed to load config.json, using defaults')
...
MDV_ASPECT_RATIO = 1.333333333 
CACHED_MDV_DATA = None
LAST_KNOWN_ARCADE_FOV = None
def getMonitorDistanceVerticalData():
    global CACHED_MDV_DATA, LAST_KNOWN_ARCADE_FOV
    arcadeFov = FovExtended.instance().actualDefaultVerticalFov
    if CACHED_MDV_DATA is None or arcadeFov != LAST_KNOWN_ARCADE_FOV:
        LAST_KNOWN_ARCADE_FOV = arcadeFov
        tanArcadeFov = math.tan(arcadeFov * 0.5)
        CACHED_MDV_DATA = (MDV_ASPECT_RATIO, math.atan(tanArcadeFov * MDV_ASPECT_RATIO))
    return CACHED_MDV_DATA
...
def __updateCrosshairMatrix(func, self):
    ...
    aspectRatio, denominator = getMonitorDistanceVerticalData()
