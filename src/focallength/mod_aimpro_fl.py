...
CACHED_ARCADE_TAN = None
LAST_KNOWN_ARCADE_FOV = None
def getArcadeTan():
    global CACHED_ARCADE_TAN, LAST_KNOWN_ARCADE_FOV
    arcadeFov = FovExtended.instance().actualDefaultVerticalFov
    if CACHED_ARCADE_TAN is None or arcadeFov != LAST_KNOWN_ARCADE_FOV:
        LAST_KNOWN_ARCADE_FOV = arcadeFov
        CACHED_ARCADE_TAN = math.tan(arcadeFov * 0.5)
    return CACHED_ARCADE_TAN
...
def __updateCrosshairMatrix(func, self):
    ...
    tanArcadeFov = getArcadeTan()
    self._SniperCamera__sensitivityMultiplier = (tanCurrentFov / tanArcadeFov)
