...
CACHED_ARCADE_FOV = None
def getArcadeFov():
    global CACHED_ARCADE_FOV
    arcadeFov = FovExtended.instance().actualDefaultVerticalFov
    if CACHED_ARCADE_FOV is None or arcadeFov != CACHED_ARCADE_FOV:
        CACHED_ARCADE_FOV = arcadeFov
    return CACHED_ARCADE_FOV
...
def __updateCrosshairMatrix(func, self):
    ...
    arcadeFov = getArcadeFov()
    self._SniperCamera__sensitivityMultiplier = (currentFov / arcadeFov)
