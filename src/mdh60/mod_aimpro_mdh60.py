...
def ensureConfigExists():
    ...
    try:
        ...
        print('[mod_aimpro_mdh60] config.json created with defaults')
    except Exception as e:
        print('[mod_aimpro_mdh60] Failed to create config.json:', e)
...
def loadConfig():
    ...
    try:
        ...
        print('[mod_aimpro_mdh60] Failed to load config.json, using defaults')
...
def getMonitorDistanceHorizontalData():
    ...
    if CACHED_MDH_DATA is None or arcadeFov != LAST_KNOWN_ARCADE_FOV or aspectRatio != LAST_KNOWN_ASPECT_RATIO:
        ...
        totalAspectRatio = (aspectRatio * 0.60)
